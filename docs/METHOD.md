># BookForge METHOD
#
# This document is the working method BookForge enforces, preserved in its
# original form: it was written as build instructions and doubled as the
# authoring discipline for the published novel. The canonical app-facing copy
# lives in the root README; this file explains WHY the app is shaped the way
# it is. Paths and examples reference the author's own trilogy; they are
# examples of data shapes, not shipped content.
...
### A22 (2026-07-24): Desk-first inline review (selection popover, suggested edits, inline anchors)

Purpose: a field report. The review surface's comment flow works but is built for a phone: you highlight a passage mid-chapter, then must scroll away to a static "Selected span" box below the entire prose to type, and existing comments have no visible anchors in the text. At a desk with a keyboard the flow should be: highlight a passage, act right there, either leave a note for the model or type the exact replacement yourself. This amendment adds an inline selection popover with two actions (Comment and Suggest edit), keyboard access throughout, mechanical application of author-typed suggestions without a model call, and inline anchor highlights for existing comments.

Design principles, non-negotiable:
- Additive UI. The existing static "Selected span" panel, the sidebar, and every existing data-testid stay byte-identical; existing e2e must pass unchanged. The popover and composer are new elements with new testids.
- A suggestion is the author's own text and applies mechanically, with NO model call. The approval gates protect against unapproved model changes; direct authorship is the same trust level as the draft editor. Plain comments keep flowing through the existing revise path unchanged.
- All offsets stay on the RAW content (the A21 invariant). The popover composer's quotedText is content.slice(lo, hi), byte-identical to the static panel's capture.

A22.1 Selection popover and inline composer:
- When a non-collapsed selection lands inside review-prose, a small floating toolbar (data-testid selection-toolbar) appears anchored near the selection, positioned from the selection Range's bounding rect inside a relatively positioned wrapper, clamped to the wrapper, above the selection and flipped below when there is no room. Toolbar buttons: Comment (toolbar-comment-button) and Suggest edit (toolbar-suggest-button). Toolbar buttons must preventDefault on mousedown so clicking them does not collapse the selection.
- Choosing an action swaps the toolbar into an inline composer card (inline-composer) at the same anchor. Comment mode: a quoted preview (rendered through the A21 EmphasisText parser) plus an autofocused textarea (inline-comment-input). Suggest mode: an autofocused textarea prefilled with the RAW selected text, markers included (inline-suggest-input), plus an optional note field (inline-note-input). One submit button (inline-composer-submit).
- Keyboard: with an active prose selection and focus outside any input, textarea, or contenteditable, pressing c opens the comment composer and e opens the suggest composer. Inside a composer, Ctrl+Enter (Cmd+Enter on Mac) submits and Escape cancels. Clicking outside the composer cancels. The popover hides when the selection collapses; the pending state feeding the static panel keeps its current last-selection behavior.
- Submission posts to the same POST /api/drafts/[id]/comments endpoint as the static panel and refreshes the sidebar list. The popover is available at all viewport sizes; the static panel remains as the fallback surface.

A22.2 Suggested edits, mechanical apply:
- Schema: comments gains a nullable suggested_text TEXT column (guarded ALTER in migrate, the D140 pattern). A row with non-null suggested_text is a suggestion; the comment column holds its optional note (may be empty). No kind enum column; nullability is the discriminator.
- POST /api/drafts/[id]/comments accepts optional suggestedText. Validation: suggestedText must be non-empty when present, must differ from quotedText, and is REJECTED with a clear 400 if it contains an em-dash (it lands verbatim in the draft; the house law applies). Plain-comment validation is unchanged; the note field is not linted.
- The revise path ignores suggestions: the flagged spans handed to the revision prompt come only from unresolved rows whose suggested_text is null (a filtered variant beside listUnresolvedComments; the lock gate keeps counting ALL unresolved rows, so an unapplied suggestion still blocks locking). The client revise button enables only when at least one unresolved plain comment exists.
- New endpoint POST /api/drafts/[id]/apply-suggestions: rejects with 409 if the draft is no longer the chapter's latest version, 400 if there are no unresolved suggestions. Anchors each unresolved suggestion via findSpan(content, quotedText, spanStart) (quoted_text is the source of truth, as everywhere). Unanchorable suggestions are skipped with reason "not found"; among anchored ones sorted by start, any suggestion overlapping an earlier-applied one is skipped with reason "overlap". All applicable replacements are applied in one transaction producing ONE new draft version. Applied suggestions are marked resolved and stay on the version they applied to; every unresolved row (plain comments and skipped suggestions) has its draft_id moved to the new draft so review continuity survives (spans recompute on load; the LLM-revision rule that comments do not carry forward is unchanged). Response: the new draft id and content, the applied count, and the skipped list with reasons (the A2.2 error-reason pattern).
- Client: the sidebar gains an Apply suggestions button (apply-suggestions-button) enabled when unapplied suggestions exist, separate from Revise flagged spans. On success the prose swaps to the new content, a summary banner (suggestions-applied) reports the applied count, and skipped suggestions render with their reasons (suggestion-skips), presentation mirroring the failed-patches block. Suggestion cards in the sidebar carry a visible badge and show the quoted text and the replacement.

A22.3 Inline anchors:
- Every unresolved comment and suggestion renders as a tinted anchor highlight over its span in the prose, comment and suggestion visually distinct, using semantic token classes only. Anchors are computed client-side with the same findSpan pure function the server uses (no API change); rows that fail to anchor simply render no highlight.
- Decoration is a pure function (new src/lib/reviewAnchors.ts): it takes the raw content and the anchor list and returns the A21 emphasis segments further split at anchor boundaries, each segment carrying its covering anchor ids. The data-raw-start invariant holds on the finer segments, so selection mapping is untouched. Clicking an anchored span scrolls the matching sidebar card into view and flashes it.

Non-goals: no comment threads or replies, no rich text notes, no MCP surface change (web UI only, the D149 stance), the draft editor and export stay raw markdown, audio untouched (a new version re-keys the content-addressed cache by itself).

Testing: unit tests for reviewAnchors decoration (anchor inside an emphasis run, anchors spanning segment boundaries, overlapping anchors, no anchors yields A21 segments verbatim), the suggestion repo and apply logic (anchoring, not-found and overlap skips, one new version, resolved rows stay put, unresolved rows move), and route validation (em-dash 400, same-text 400, stale-draft 409, empty 400, revise excludes suggestions). New a22 e2e: selection produces the toolbar; the comment path stores a comment and shows its anchor; the suggest path prefills, submits, applies, and the prose updates; a not-found suggestion is skipped and reported; c and e open composers and Escape closes; the revise button stays disabled when only suggestions are pending. Every existing spec stays green unchanged.

Acceptance check: on the deployed app, highlight a sentence mid-chapter and a toolbar appears at the selection; press c, type a note, Ctrl+Enter, and the comment lands with a visible highlight in the prose; highlight a phrase, press e, type replacement text, submit, Apply suggestions, and the prose updates to the author's exact words with no model call; the phone flow (static panel) still works exactly as before.

### A23 (2026-07-25): Security hardening

Purpose: a five-surface security review of the deployed app (auth and session, API and data layer, the LLM subprocess boundary, the client, and the box) produced a set of confirmed defects. This amendment fixes the ones that live in this repo. It is hardening only: no feature changes, no UI changes beyond response headers, and every existing test stays green unchanged.

Threat model, stated once so the fixes can be judged against it: the app is on the public internet behind ONE shared password. It holds the author's unpublished manuscript (confidentiality), spends real money and CPU on model calls (abuse), and runs as root on a box beside other services (blast radius). There is no multi-tenancy, so per-user authorization is not a concern; what matters is unauthenticated access, availability, and anything that lets untrusted text reach a shell, a path, or an approval gate.

A23.1 Response headers (the clickjacking fix): the app sets no security headers today, and no destructive control in the UI has a confirmation step, so a single framed click can delete a canon fact or start a paid model run. next.config.ts gains a headers() block applying to every path: Content-Security-Policy with frame-ancestors 'none', object-src 'none', base-uri 'self', form-action 'self'; X-Frame-Options DENY; X-Content-Type-Options nosniff; Referrer-Policy same-origin; Permissions-Policy granting microphone to self (the voice recorder needs it) and denying camera, geolocation, and payment; Strict-Transport-Security with a one year max-age. A script-src policy is deliberately out of scope: it needs a per-request nonce, and the review found no HTML injection sink anywhere in the app, so it is defense in depth rather than a fix.

A23.2 Cross-site request rejection: the session cookie is SameSite=Lax on a host under a domain that hosts other services, so a sibling subdomain is same-site and can drive writes. The middleware rejects any request whose Sec-Fetch-Site header is exactly "cross-site" with a 403, after the session check. Same-origin requests report "same-origin", typed URLs and bookmarks report "none", so this blocks only genuine cross-site traffic. This also closes the one live cross-site vector: the audio route is a GET that synthesizes speech, and Lax sends the cookie on top-level GET navigation.

A23.3 Session and password handling in src/lib/auth.ts:
- A malformed cookie crashes the gate: fromBase64Url runs atob outside the try block, so a cookie like "x.!" throws and every gated path returns 500. Both halves of the token are validated against /^[A-Za-z0-9_-]+$/ before decoding. Fails closed as before, but with a 401 instead of an unauthenticated 500 generator.
- passwordMatches compares raw bytes and returns early on a length mismatch, leaking the password length under repeated timing samples. It compares HMAC digests of the two inputs instead, which are 32 bytes regardless of input, and becomes async (its one caller is already async).
- A session survives a password change: the token is keyed only on SESSION_SECRET, so rotating APP_PASSWORD after a suspected compromise revokes nothing. The signing key becomes a function of BOTH secrets, so rotating either invalidates every live session. Consequence to accept knowingly: deploying this signs the author out once. The maximum age drops from 30 days to 7.

A23.4 The LLM subprocess boundary in src/lib/llm/client.ts. Command injection is NOT present on the production path (Linux always spawns with shell false, and the prompt rides stdin, never argv), and that property is preserved. The fixes are:
- An unhandled EPIPE on the child's stdin crashes the server. When the CLI exits before draining stdin (a bad model id, expired CLI auth, an unknown flag after an upgrade), writing the prompt emits an error on a stream with no listener, which is an uncaught exception that takes down the Node process. One authenticated request triggers it. An 'error' listener is installed on child.stdin exactly as src/lib/audio/ffmpeg.ts already does, letting the close handler report the failure.
- The Windows fallback spawns with shell true, where argv values are not quoted, so a model name or POV string containing shell metacharacters would execute. The model id is validated at the settings boundary and again where argv is built, against /^[A-Za-z0-9._:-]+$/. The Windows path is left working (it is the author's dev machine) but can no longer carry metacharacters.
- fixtureKey is taken verbatim from nine request bodies and interpolated into a fixture filename. It is only read when USE_FIXTURE_LLM is set, so production is unaffected, but it is validated against /^[A-Za-z0-9._-]+$/ at the loadFixture boundary regardless.

A23.5 Control frame integrity: the streaming protocol separates prose from its control frame with a delimiter that "will never occur in generated prose", but nothing enforces that and every consumer uses indexOf. Manuscript text instructing the model to emit that literal string truncates what the author sees before approving a revision. All three consumers (src/lib/chatStream.ts, src/components/DraftEditor.tsx, src/components/ReviewEditor.tsx) switch to lastIndexOf. The server always writes the genuine frame last, so lastIndexOf is strictly correct.

A23.6 Lock gate integrity: POST /api/chapters/[id]/lock refuses to lock while any comment is unresolved, but PATCH /api/chapters/[id] accepts an arbitrary status field including "locked", bypassing that check. The PATCH route rejects a status of "locked", directing callers to the lock endpoint that enforces the gate. This restores the invariant that locking is a gated, human-only action rather than one enforced in a single place and open in another.

A23.7 Login throttling: the login endpoint has no rate limit, lockout, delay, or failure logging, and the deployed Caddy log shows the box being actively probed. The password is long and random, so this is not a credential-compromise risk today; it is an unmetered request cost and a missing detection signal. A pure decideLogin(state, key, now) function (unit tested, the repo's usual shape) backs a per-IP and global counter held in module memory, which is correct because the deploy pins a single instance. Failures return 429 with Retry-After and log one warning line.

A23.8 Crash reporting hygiene: a failed JSON parse in the browser produces an error message containing a slice of the response body, which can be manuscript text, and the crash client ships error messages off-box. A beforeSend hook truncates the exception value. The review confirmed the client never reads cookies and never captures request or response bodies, so no session material was ever at risk.

A23.9 Source map exposure: productionBrowserSourceMaps is on and the uploader deletes the browser maps only when passed a flag, so any deploy that forgets it publishes the app's client source to the internet. This actually happened. The uploader deletes uploaded browser maps BY DEFAULT, with an explicit --keep-browser-maps opt-out, so the unsafe outcome now requires a deliberate flag rather than a remembered one.

A23.10 Unused image optimizer: the middleware matcher excludes /_next/image, and the app uses no next/image anywhere, so the exclusion only exposes an unauthenticated endpoint carrying its own advisories. It is removed from both the matcher and isStaticAsset. Verified before the change that the endpoint could not invoke gated handlers.

Dependencies: next moves to 15.5.21, a patch release closing eight advisories (SSRF in rewrites and Server Actions, cache confusion, DoS, and unauthenticated disclosure of internal Server Function endpoints). Other advisories in the tree are dev-only (the vitest UI server, the vite and esbuild dev servers) and are not reachable in production.

Out of scope, recorded deliberately: a nonce-based script-src CSP; per-route session checks as defense in depth behind the middleware; and every box-level finding (service exposure, file permissions, off-box backups), which are operational changes for the owner rather than repo changes.

Testing: unit tests for the login throttle decision function, the token and password changes in auth, and the fixture-key and model-id validators. The full existing suite must stay green with no assertion weakened. Acceptance: the deployed site serves the new headers, a cross-site POST is refused, a malformed session cookie yields 401 rather than 500, and login still works.
